...
def trigger_live_weather_refresh(db: Session = Depends(get_db)):
    """
    Trigger synchronized multi-source polling across Open-Meteo, IMD, DEM terrain,
    and OSM Overpass, updating weather readings and recalculating AI risk scores.
    """
    sync_res = ingestion_pipeline.run_full_ingestion_sync(db, auto_dispatch_alerts=False)
    return {
        "status": "success",
        "zones_refreshed": sync_res.get("zones_processed", 0),
        "timestamp": sync_res.get("synced_at"),
        "imd_bulletin": sync_res.get("imd_bulletin"),
        "sources_integrated": sync_res.get("sources_integrated")
    }
...
def get_zone_forecast(zone_id: int, db: Session = Depends(get_db)):
    ...
    for step in forecast_series:
        ...
        augmented_series.append({
            ...
            "wind_speed_kmh": step.get("wind_speed_kmh", 14.0),
